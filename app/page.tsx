...
export default function DashboardAgrimensura() {
  ...
  const [nuevaFinanza, setNuevaFinanza] = useState({ 
    tramite: "VEP", propietario: "", encargado: "Leo", ingreso: 0, caja: 83000, colegio: 69300, extraLeo: 20800, extraBruno: 0 
  });
        ...
        <div className="space-y-6">
          <form onSubmit={guardarFinanza} className="bg-white p-4 rounded-lg shadow border grid grid-cols-4 gap-4 items-end">
            ...
            <div className="col-span-4 flex justify-end gap-2 mt-2">
              {editandoFinanzaId && (<button type="button" onClick={() => {setEditandoFinanzaId(null); setNuevaFinanza({ tramite: "VEP", propietario: "", encargado: "Leo", ingreso: 0, caja: 83000, colegio: 69300, extraLeo: 20800, extraBruno: 0 });}} className="px-6 py-2 rounded font-bold text-slate-600 bg-slate-200 hover:bg-slate-300">Cancelar</button>)}
              <button type="submit" className={`px-6 py-2 rounded font-bold text-white ${editandoFinanzaId ? 'bg-orange-500 hover:bg-orange-600' : 'bg-blue-600 hover:bg-blue-700'}`}>{editandoFinanzaId ? "Guardar Edición" : "Cargar Ingreso"}</button>
            </div>
          </form>
          ...
          <div className="bg-slate-800 text-white rounded-lg shadow-lg p-6 flex items-center justify-between">
            <div className="flex gap-12">
              <div>
                ...
                <p className="text-sm border-b border-slate-600 pb-1 mb-1">Gastos: ${formatearPlata(gastosLeo)}</p>
              ...
              <div>
                ...
                <p className="text-sm border-b border-slate-600 pb-1 mb-1">Gastos: ${formatearPlata(gastosBruno)}</p>
